...
from tests.utils import TEST_MEDIA_DIR, get_test_image_buffer, get_test_image_file
...
class TestStoryPage(TestCase):
    ...
    def test_render_page(self):
        ...
        self.assertContains(
            response,
            'src="http://media.example.com/media/images/mountain-wagtail.original.png"',
        )
        ...
        self.assertContains(
            response,
            'src="http://media.example.com/media/media/wagtail-in-flight.webm"',
        )
    ...
    def test_import_images(self):
        ...
        responses.add(
            responses.GET,
            "https://example.com/torchbox.png",
            content_type="image/png",
            body=get_test_image_buffer(colour="purple", size=(64, 64)).getvalue(),
        )
        poster_portrait_data = get_test_image_buffer(
            colour="black", format="JPEG", size=(640, 853)
        ).getvalue()
        poster_square_data = get_test_image_buffer(
            colour="blue", format="JPEG", size=(640, 640)
        ).getvalue()
        poster_landscape_data = get_test_image_buffer(
            colour="green", format="JPEG", size=(853, 640)
        ).getvalue()

        responses.add(
            responses.GET,
            "https://example.com/wagtails-portrait.jpg",
            content_type="image/jpeg",
            body=poster_portrait_data,
        )
        responses.add(
            responses.GET,
            "https://example.com/wagtails-square.jpg",
            content_type="image/jpeg",
            body=poster_square_data,
        )
        responses.add(
            responses.GET,
            "https://example.com/wagtails-landscape.jpg",
            content_type="image/jpeg",
            body=poster_landscape_data,
        )
        responses.add(
            responses.GET,
            "https://example.com/pied-wagtail.jpg",
            content_type="image/jpeg",
            body=get_test_image_buffer(
                colour="yellow", format="JPEG", size=(320, 240)
            ).getvalue(),
        )
        responses.add(
            responses.GET, "https://example.com/broken.jpg", body=HTTPError("not found")
        )
        responses.add(
            responses.GET,
            "https://example.com/wagtail-poster.png",
            content_type="image/png",
            body=get_test_image_buffer(colour="yellow", size=(600, 800)).getvalue(),
        )
        responses.add(
            responses.GET,
            "https://example.com/wagtail-in-flight.mp4",
            content_type="video/mp4",
            body="pretend this is a video",
        )
        responses.add(
            responses.GET, "https://example.com/broken.mp4", body=HTTPError("not found")
        )
        ...
        self.assertEqual(
            story_page.get_poster_portrait_rendition().file.read(), poster_portrait_data
        )
        self.assertEqual(
            story_page.get_poster_square_rendition().file.read(), poster_square_data
        )
        self.assertEqual(
